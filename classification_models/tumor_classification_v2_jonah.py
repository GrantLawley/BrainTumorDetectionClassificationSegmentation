...
# define model
# slow and performance is terrible
class ResidualBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        # Add the original input (identity) back to the output
...
class ResNet(nn.Module):
    def __init__(self, num_classes=4):
        ...
        # Initial Layer (224x224)
        self.conv1 = nn.Conv2d(1, 32, kernel_size=3, stride=1, padding=1, bias=False)
        self.bn1 = nn.BatchNorm2d(32)
        
        # Residual Layers
        self.layer1 = ResidualBlock(32, 32, stride=1)
        self.layer2 = ResidualBlock(32, 64, stride=2) # Downsample to 112x112
        self.layer3 = ResidualBlock(64, 128, stride=2) # Downsample to 56x56
...
def train_eval(nn_model,train_loader,test_loader,name,epochs=2):
    ...
    for epoch in range(epochs):
        ...
        if avg_val_loss < best_loss:
            ...
            trigger_times = 0
            best_model_path = os.path.join(proj_dir, "results", task, "tumor_classification_v2_jonah", f"{name}_best.pt")
...
if __name__ == "__main__":
    ...
    task = "classification"
    os.makedirs(os.path.join(proj_dir, "results", task, "tumor_classification_v2_jonah"), exist_ok=True)
    png_out = os.path.join(proj_dir, "results", task, "tumor_classification_v2_jonah", "convergence.png")
    json_out = os.path.join(proj_dir, "results", task, "tumor_classification_v2_jonah", "metrics.json")
